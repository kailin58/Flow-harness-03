...
```
触发 escalate（5路来源）
  │  A. orchestration.collab（L13）状态机 on_timeout = escalate
  │  B. quality.gate（L18）门禁层打回次数 ≥ N（默认3次）
  │  C. release.publish（L22）发布层审批等待超时
  │  D. 工具调用风险等级=极高
  │  E. 系统置信度低于阈值
  │
  ├─ 1. 任务状态冻结
...
人工可以：覆盖 quality.gate（L18）门禁决策 / 修改任务状态 / 终止任务 / 强制降级
人工不可以：绕过 S3 审计写入 / 直接修改 evolution.experiment（L23）进化结果
```
