...
class CICDIntegration {
  ...
  generateGitLabCI(options = {}) {
    const nodeVersion = options.nodeVersion || '18';
    const stages = ['install', 'test', 'quality', 'deploy'];
    ...
    if (this.qualityGates.length > 0) {
      config['quality-gate'] = {
        stage: 'quality',
        script: this.qualityGates.map(g => g.command),
        allow_failure: false
      };
    }
    ...
    for (const [envName, envConfig] of this.environments) {
      const deployJobName = `deploy-${envName}`;
      config[deployJobName] = {
        stage: 'deploy',
        script: [
        ...
        rules: [
          { if: `$CI_COMMIT_BRANCH == "${envConfig.branch}"`, when: 'on_success' }
        ],
        environment: {
          name: envName,
          url: `https://${envName}.example.com`
        }
      };

      if (envConfig.requiresApproval) {
        config[deployJobName].when = 'manual';
      }
    }

    const content = this._toYaml(config);
    return { content, path: path.join(this.projectDir, '.gitlab-ci.yml') };
  }
  ...
  generateJenkinsfile(options = {}) {
    ...
    let content = `// Jenkinsfile - Generated by Flow Harness
...
`;
      for (const gate of this.qualityGates) {
        const failStep = gate.required ? '' : 'true';
        content += `          sh '${gate.command}' || ${failStep} // ${gate.name}\n`;
      }
      content += `        }
      }
    }
...
`;
    ...
    return { content, path: path.join(this.projectDir, 'Jenkinsfile') };
  }
  ...
  /**
   * 根据平台生成工作流
   * @param {string} platform - github/gitlab/jenkins
   * @param {Object} options
   * @returns {Object} { content, path }
   */
  generateForPlatform(platform, options = {}) {
    switch (platform) {
      case 'gitlab':
      ...
      case 'github':
      default:
        return this.generateCI(options);
    }
  }
   ...
   * 生成所有平台配置
   * @param {Object} options
   * @returns {Array<{content, path, platform}>}
   */
  generateAllPlatforms(options = {}) {
    const results = [];
    const platforms = ['github', 'gitlab', 'jenkins'];

    for (const platform of platforms) {
      try {
        const result = this.generateForPlatform(platform, options);
        results.push({ ...result, platform });
      } catch (error) {
        this.logger.warn({ platform, error: error.message }, 'Failed to generate platform config');
      }
    }
