...
Flow Harness 采用 **5层固定组织架构**：CEO → VP（11个）→ 总监 → 子Agent → **副Deep Agent**（L4，按需孵化、任务结束即销毁）。
任何 Agent 执行端（Claude Code / Cursor / Codex）都必须遵守此架构。层级定义与 `meeting/20260414升级总结/01_3层升4层架构.md` 一致（3层→4层→**5层**终态）。
...
│    └───────────┘                                                  │
│                                                                  │
│  注：CEO(1)+VP(11)+总监(N)+子Agent(N)+副Deep(M 按需) — 层数写死  │
└──────────────────────────────────────────────────────────────────┘
```
...
5. **统一接口**: 遵循相同的输入输出格式
6. **副Deep Agent（L4）**: 挂载在子Agent之下、按需孵化；负责单一原子任务的极深递归处理，**不再向下分发**；子任务复杂度超阈值时由子Agent孵化，任务完成即销毁（与 `01_3层升4层架构.md` 一致）
...
子Agent（拆解与协调；必要时孵化副Deep Agent）
   │
   ├─（可选）▶ 副Deep Agent（L4，单点深挖，Ephemeral）
   │
   ▼
结果逐层上报 → CEO 检查和复盘
```
...
1. **5层固定架构**: CEO → VP（11个）→ 总监 → 子Agent → 副Deep Agent（按需），层数写死
2. **VP是唯一出口**: 跨VP通信必须通过VP层，不可绕过
3. **CEO只做匹配**: 路由基于S4能力标签注册表，不做智能推断
4. **总监只执行**: 不越级、不跨域、结果逐层上报
...
- **Flexible Extension**: 总监、子Agent 按需扩展，子Agent 下 **副Deep** 按需孵化；VP 总数固定（11）

---

**版本**: 2.2
**最后更新**: 2026-04-15
**状态**: 正式版（5层架构：CEO + 11VP + N总监 + N子Agent + 副Deep按需；含 S3 审计写入缺口8约定）
