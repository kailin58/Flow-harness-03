...
Flow Harness 采用 **4层固定组织架构**：CEO → VP（11个）→ 总监 → 子Agent。
...
│    └───────────┘                                                  │
│                                                                  │
│  注：CEO(1) + VP(11) + 总监(N) + 子Agent(N) — 层数固定，数量弹性  │
└──────────────────────────────────────────────────────────────────┘
```
...
## 副Deep Agent 定位说明（运行时计算单元）

> **重要**：副Deep Agent **不是第5个永久组织层级**，组织架构固定为4层。

副Deep Agent 是子Agent 在执行期可按需孵化的**原子任务执行单元**，属于运行时概念：

| 属性 | 说明 |
|------|------|
| **挂载位置** | 挂载于 L3 子Agent 之下，按需孵化 |
| **角色** | 深度执行单元，负责单一原子任务的极深递归处理 |
| **生命周期** | 任务型（ephemeral）：单次任务完成即销毁，不长驻 |
| **与子Agent区别** | 子Agent 负责任务拆解与协调；副Deep Agent 只做单点深挖，不再向下分发 |
| **触发条件** | 子Agent 判断某子任务复杂度超阈值时，主动孵化一个副Deep Agent 处理 |
| **通信规则** | 副Deep Agent 只能被动响应上级子Agent，不可主动越级上报；不参与层级通信规则的计算 |

**为何不算第5层**：副Deep Agent 没有持久注册表、没有固定ID、任务结束即销毁，不构成可路由的组织节点。通信规则、权限控制、路由决策均以4层组织为基准，副Deep Agent 对外不可见。
...
1. **4层固定架构**: CEO → VP（11个）→ 总监 → 子Agent，层数写死
...
- **Flexible Extension**: 总监和子Agent按需扩展，VP总数固定

---

**版本**: 2.0
**最后更新**: 2026-04-13
**状态**: 正式版（4层架构：CEO + 11VP + N总监 + N子Agent）
