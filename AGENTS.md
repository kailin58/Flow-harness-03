...
Flow Harness 采用 **1 CEO + 5 总监** 的固定组织架构，任何 Agent 执行端（Claude Code / Cursor / Codex）都必须遵守此架构。
...
│  ┌──▼───┐  ┌──▼───┐  ┌───▼──┐  ┌───▼──┐  ┌───▼──┐       │
│  │总监1  │  │总监2  │  │总监3  │  │总监4  │  │总监5  │       │
│  │Explor│  │ Plan │  │Genera│  │Inspec│  │Resear│       │
│  │Agent │  │Agent │  │Agent │  │Agent │  │Agent │       │
│  └──────┘  └──────┘  └──────┘  └──────┘  └──────┘       │
│                                                             │
│  注：CEO + 5个总监 = 6个角色，写死，不可增减                  │
└─────────────────────────────────────────────────────────────┘
```
...
#### Research Agent 的子Agent
- **WebSearchAgent**: 网络搜索
- **DocFetchAgent**: 文档抓取
- **ApiLookupAgent**: API 查询
- **KnowledgeRetrievalAgent**: 知识库检索
...
CEO (Supervisor Agent)
   ...
   │        │        │        │        │        │
   ▼        ▼        ▼        ▼        ▼        ▼
Explore   Plan   General  Inspector Research
Agent    Agent   Agent     Agent     Agent
   │        │        │        │        │
   └────────┴────────┴────────┴────────┘
                    │
...
```json
{
  "agents": {
    ...
    "research": {
      "name": "Research Agent",
      "role": "总监5",
      "capabilities": ["web_search", "fetch_url", "doc_lookup", "api_reference", "knowledge_retrieval"],
      "status": "active"
    }
  }
}
